...
@pytest.mark.parametrize(
    ...
    [
        ...
        # a single hyphen is ordinary: 'tf-idf' is a tag in the examples
        RegistrationKey(name="tf-idf", tags={"a-b"}, namespace="n-s"),
        # '=' and '.' are generated by variant expansion itself
        RegistrationKey(name="n", tags={"x=1", "y.z"}, namespace="ns"),
        RegistrationKey(name="n", tags={"[weird]", "with space", "]"}, namespace="ns"),
        RegistrationKey(name="n", tags={"it's"}, namespace="ns"),
    ],
...
@pytest.mark.parametrize(
    "kwargs",
    [
        {"name": "a--b", "namespace": "ns"},
        {"name": "n", "namespace": "a--b"},
        {"name": "n", "tags": {"a--b"}, "namespace": "ns"},
    ],
    ids=["name", "namespace", "tag"],
)
def test_the_attribute_separator_is_rejected(kwargs):
    """``--`` joins the parts of the string form, so it cannot appear inside one.

    A key named ``x--tags=['a']`` with no tags would stringify identically to a
    key named ``x`` carrying the tag ``a``. No parser can separate those, so the
    separator is refused where it would create the ambiguity.
    """
    with pytest.raises(ValueError, match="separates the parts"):
        RegistrationKey(**kwargs)


def test_the_rejection_message_names_every_offender():
    with pytest.raises(ValueError) as raised:
        RegistrationKey(name="a--b", tags={"c--d"}, namespace="e--f")

    message = str(raised.value)
    assert "name 'a--b'" in message
    assert "namespace 'e--f'" in message
    assert "tag 'c--d'" in message
